#!/usr/bin/env node
/**
 * Repair Partners i18n gaps: footer labels + legal phrases still in English.
 * Also restores protected tokens mangled by MT (e.g. "__ KP_0 __").
 */
...
const LANG = {
  af: 'af', am: 'am', ar: 'ar', 'ar-dz': 'ar', 'ar-ma': 'ar', 'ar-tn': 'ar', az: 'az', be: 'be',
  bg: 'bg', bn: 'bn', cs: 'cs', cy: 'cy', da: 'da', de: 'de', 'de-be': 'de', el: 'el', es: 'es',
  et: 'et', fa: 'fa', fi: 'fi', fil: 'tl', fr: 'fr', 'fr-be': 'fr', ga: 'ga', gu: 'gu', ha: 'ha',
  he: 'he', hi: 'hi', hr: 'hr', hu: 'hu', hy: 'hy', id: 'id', ig: 'ig', is: 'is', it: 'it',
  ja: 'ja', ka: 'ka', kk: 'kk', km: 'km', kn: 'kn', lb: 'lb', lo: 'lo', lt: 'lt', lv: 'lv',
  mk: 'mk', ml: 'ml', mn: 'mn', mr: 'mr', ms: 'ms', mt: 'mt', my: 'my', ne: 'ne', nl: 'nl',
  'nl-be': 'nl', no: 'no', pa: 'pa', pl: 'pl', pt: 'pt', 'pt-br': 'pt', ro: 'ro', ru: 'ru',
  si: 'si', sk: 'sk', sl: 'sl', so: 'so', sq: 'sq', sr: 'sr', sv: 'sv', sw: 'sw', ta: 'ta',
  te: 'te', tg: 'tg', th: 'th', tr: 'tr', uk: 'uk', ur: 'ur', uz: 'uz', vi: 'vi', yo: 'yo',
  zh: 'zh-CN', 'zh-tw': 'zh-TW', zu: 'zu',
};
...
const MANUAL_FOOTER = {
  ...
  fil: { partners: 'Mga Partner', affiliateProgram: 'Programang Affiliate', becomePartner: 'Maging Partner' },
  ...
  ne: { partners: 'पार्टनरहरू', affiliateProgram: 'Affiliate कार्यक्रम', becomePartner: 'पार्टनर बन्नुहोस्' },
  si: { partners: 'හවුල්කරුවන්', affiliateProgram: 'අනුබද්ධ වැඩසටහන', becomePartner: 'හවුල්කරුවෙකු වන්න' },
  sw: { partners: 'Washirika', affiliateProgram: 'Programu ya washirika', becomePartner: 'Kuwa mshirika' },
  so: { partners: 'Lammaanayaasha', affiliateProgram: 'Barnaamijka lammaanaha', becomePartner: 'Noqo lammaane' },
  yo: { partners: 'Awọn alabaṣepọ', affiliateProgram: 'Ètò alabaṣepọ', becomePartner: 'Di alabaṣepọ' },
  zu: { partners: 'Abalingani', affiliateProgram: 'Uhlelo lwabalingani', becomePartner: 'Yiba umlingani' },
  gu: { partners: 'ભાગીદારો', affiliateProgram: 'એફિલિએટ પ્રોગ્રામ', becomePartner: 'ભાગીદાર બનો' },
  ha: { partners: 'Abokan hulɗa', affiliateProgram: 'Shirin abokan hulɗa', becomePartner: 'Zama abokin hulɗa' },
  ig: { partners: 'Ndị mmekọ', affiliateProgram: 'Mmemme ndị mmekọ', becomePartner: 'Bụrụ onye mmekọ' },
  ...
  cy: { partners: 'Partneriaid', affiliateProgram: 'Rhaglen bartneriaeth', becomePartner: 'Dod yn bartner' },
  ga: { partners: 'Comhpháirtithe', affiliateProgram: 'Clár comhpháirtíochta', becomePartner: 'Bí i do chomhpháirtí' },
};
...
function restore(text, tokens) {
  ...
  tokens.forEach((value, i) => {
    ...
    // Repair common MT mangling around tokens.
    out = out.split(`⟦ ${i} ⟧`).join(value);
    out = out.split(`[${i}]`).join(value);
  });
  // Repair older mangled KP tokens from previous run.
  out = out.replace(/__\s*KP_(\d+)\s*__/g, (_, n) => tokens[Number(n)] ?? KEEP[Number(n)] ?? '');
  return out;
}

let googleTranslate;
function getGoogle() {
  if (!googleTranslate) googleTranslate = require('@vitalets/google-translate-api').translate;
...
async function translate(text, code) {
  ...
  try {
    const result = await Promise.race([
      getGoogle()(out, { from: 'en', to: target, requestOptions: { timeout: 10000 } }),
      sleep(11000).then(() => null),
    ]);
    const translated = restore(String(result?.text ?? '').trim(), tokens);
...
function patchFooter(code, footer) {
...
function needsLegalFix(value, english) {
  if (!value) return true;
  if (value === english) return true;
  if (/__\s*KP_\d+\s*__/.test(value)) return true;
  if (/⟦\s*\d+\s*⟧/.test(value) && value.includes('⟦')) {
    // leftover unrestored tokens
    if (!KEEP.some((k) => value.includes(k)) && /⟦/.test(value)) return true;
  }
  return false;
}

async function repairLegal(code) {
  ...
  for (const phrase of LEGAL_PHRASES) {
    if (!needsLegalFix(map[phrase], phrase)) continue;
    const translated = await translate(phrase, code);
...
async function main() {
  ...
  // 1) Footer labels from curated map / parent / MT
  for (const code of locales) {
    const parent = VARIANT[code];
    const footer = MANUAL_FOOTER[code] ?? (parent ? MANUAL_FOOTER[parent] : null);
    if (footer) {
      patchFooter(code, footer);
      console.log(`footer ${code}: curated`);
      continue;
    }
    const translated = {
      partners: await translate(FOOTER_EN.partners, code),
      affiliateProgram: await translate(FOOTER_EN.affiliateProgram, code),
      becomePartner: await translate(FOOTER_EN.becomePartner, code),
    };
    patchFooter(code, translated);
    console.log(`footer ${code}: ${translated.partners}`);
    await sleep(150);
  }
  ...
  for (const code of legalCodes) {
    const changed = await repairLegal(code);
    console.log(`legal ${code}: +${changed}`);
  }
