...
const data = [
  ...
  ['GL', 'Greenland', '+299'], ['GD', 'Grenada', '+1'], ['GP', 'Guadeloupe', '+590'],
  ['GU', 'Guam', '+1'], ['GT', 'Guatemala', '+502'], ['GN', 'Guinea', '+224'],
  ...
  ['NE', 'Niger', '+227'], ['NG', 'Nigeria', '+234'], ['MK', 'North Macedonia', '+389'],
  ...
  ['PR', 'Puerto Rico', '+1'], ['QA', 'Qatar', '+974'], ['RE', 'Reunion', '+262'],
  ...
  ['TM', 'Turkmenistan', '+993'], ['TC', 'Turks and Caicos Islands', '+1'], ['UG', 'Uganda', '+256'],
  ...
  ['VG', 'Virgin Islands (British)', '+1'], ['VI', 'Virgin Islands (US)', '+1'],
  ['YE', 'Yemen', '+967'], ['ZM', 'Zambia', '+260'], ['ZW', 'Zimbabwe', '+263'],
];

data.sort((a, b) => a[1].localeCompare(b[1]));

const countries = data.map(([code, name, dial]) => ({ code, name, dial }));

const content = `export interface PhoneCountry {
  code: string;
  name: string;
  dial: string;
}

/** ${countries.length} countries, sorted alphabetically by name */
export const PHONE_COUNTRIES: PhoneCountry[] = ${JSON.stringify(countries, null, 2)};

export function findPhoneCountry(code: string): PhoneCountry | undefined {
  return PHONE_COUNTRIES.find((country) => country.code === code.toUpperCase());
}
...
export function filterPhoneCountries(query: string): PhoneCountry[] {
  ...
  const dialQuery = q.startsWith('+') ? q : q.replace(/[^0-9+]/g, '');
  ...
  return PHONE_COUNTRIES.filter((country) => {
    ...
    if (dialQuery && country.dial.includes(dialQuery)) return true;
    return false;
  });
}
`;

writeFileSync('src/data/phoneCountries.ts', content);
console.log(`Generated ${countries.length} phone countries`);
